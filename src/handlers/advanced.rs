...
use crate::models::response::{
    SubsonicResponse, ResponseContainer, NowPlaying, NowPlayingEntry,
    ChatMessages, ChatMessage, Videos, Video, VideoInfo, Hls,
};
...
) -> Result<Json<SubsonicResponse<NowPlaying>>, AppError> {
    // 查询最近15分钟内播放的歌曲
...
pub async fn get_system_info(
    ...
    Query(_params): Query<CommonParams>,
) -> Result<Json<SubsonicResponse<SystemInfoResponse>>, AppError> {
    // 获取音乐库统计
...
) -> Result<Json<SubsonicResponse<Videos>>, AppError> {
...
) -> Result<Json<SubsonicResponse<VideoInfo>>, AppError> {
...
) -> Result<Json<SubsonicResponse<Hls>>, AppError> {
    // HLS 流需要生成 m3u8 播放列表
    // 这里简化处理，返回一个 URL
    let url = format!("/rest/stream?id={}", params.id);
